...
// [2] 아이콘 경로 (이 부분은 React 컴포넌트에 포함되어 있지만, quoteLogic.jsx가 독립적이라면 이 위치에 둘 수 있습니다. 여기서는 계산에 필요한 부분만 유지하겠습니다.)
...
/**
 * 시공 구역의 기본 단가를 가져옵니다.
 * 에폭시(kerapoxy) 선택 시 오버라이드 가격을 적용합니다.
 */
...
/**
 * 최종 견적을 계산하는 핵심 함수입니다.
 * 패키지 할인, 최소 비용, 재료별 가격을 모두 고려합니다.
 */
export const calculateEstimate = (quantities, housingTypeId, materialId, selectedReviews) => {
    ...
    allAreasCombined.forEach(area => {
        ...
        if (count > 0) {
            ...
            // 폴리아스파틱의 경우 기본 단가에 priceMod를 곱함 (현재 priceMod가 1.0이므로 큰 영향 없음)
    ...
    // (패키지 로직은 기존 코드를 그대로 유지하여, 잔여 항목 계산 시 중복 계산 방지)
    ...
    [...SERVICE_AREAS, ...SILICON_AREAS].forEach(area => {
        ...
        if (count > 0) {
            ...
            // 3-1. 기본 단가 (getBsaePrice는 에폭시 오버라이드 가격을 이미 반영함)
            let price = getBasePrice(area.id, materialId);
            
            // 3-2. 수량 및 주거 유형 계수 곱하기
            price *= count * selectedHousing.multiplier;
            
            // 3-3. 패키지 적용 시 잔여 항목에 대한 추가 할인/가격 조정
            if (isPackageActive) {
    ...
    // 선택된 항목이 1개이고, 패키지 후 가격이 20만원 미만일 때 (0보다 커야 함)
