...
// =================================================================
// [스타일] 애니메이션 정의 (화이트 모던 & 시인성 최우선 컨셉)
// =================================================================
const GlobalStyles = () => (
  <style>{`
    ...
    .selection-selected {
      border: 3px solid #10B981 !important; /* 민트 그린 강조 */
      background-color: #ECFDF5 !important; /* Emerald 50 */
      box-shadow: 0 4px 10px rgba(16, 185, 129, 0.2); /* 은은한 컬러 그림자 */
    }
    .safe-area-bottom { padding-bottom: env(safe-area-inset-bottom); }

    /* 클린 스크롤바 */
    .custom-scrollbar::-webkit-scrollbar { width: 6px; }
    .custom-scrollbar::-webkit-scrollbar-thumb { background-color: #10B981; border-radius: 10px; border: 1px solid #FFFFFF; }
...
const MATERIALS = [
  ...
  { 
    ...
    badge: 'Premium', badgeColor: 'bg-[#10B981]/20 text-[#10B981] border border-[#10B981]/50'
  },
];
...
const PackageToast = ({ isVisible, onClose, label }) => {
    ...
    return (
        <div className="fixed bottom-[120px] left-1/2 -translate-x-1/2 z-50 max-w-sm w-11/12">
            {/* 민트 그린 배경, 화이트 악센트 */}
            <div className="bg-[#10B981] text-white p-3 rounded-xl shadow-lg border border-emerald-600 flex items-center justify-between animate-toast">
                ...
                <button 
                    onClick={onClose} 
                    className="text-xs font-extrabold bg-white text-[#10B981] px-2.5 py-1 rounded-full hover:bg-gray-100 transition active:scale-95 flex-shrink-0 shadow-md"
                >
                    확인
                </button>
            </div>
...
const MaterialDetailModal = ({ onClose }) => (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white text-[#374151] w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-slide-down border border-gray-200">
        <div className="bg-[#10B981] p-4 text-white flex justify-between items-center border-b border-emerald-600">
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-3 text-center font-extrabold text-[#10B981]">에폭시</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              <tr className="hover:bg-gray-50">
                ...
                <td className="px-3 py-3 text-center font-bold text-[#10B981]">최상 (5년 보장)</td>
              </tr>
              <tr className="hover:bg-gray-50">
                ...
                <td className="px-3 py-3 text-center font-bold text-[#10B981]">5년</td>
              </tr>
              <tr className="hover:bg-gray-50">
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200">
          <button onClick={onClose} className="w-full py-3 bg-[#10B981] text-white rounded-lg font-bold hover:bg-emerald-600 transition active:scale-95 shadow-md">확인</button>
        </div>
      </div>
    </div>
);
...
const Accordion = ({ question, answer }) => {
    ...
    return (
        <div className="border-b border-gray-200">
            <button
                className="flex justify-between items-center w-full py-3 text-left font-extrabold text-gray-700 hover:text-[#10B981] transition duration-150"
                onClick={() => setIsOpen(!isOpen)}
            >
                <span>{question}</span>
                <ChevronDown size={18} className={`text-gray-500 transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#10B981]' : ''}`} />
            </button>
            {isOpen && (
                <div className="pb-3 text-sm text-gray-600 animate-slide-down bg-gray-50 p-4 rounded-lg -mt-1 mb-2 border-l-4 border-[#10B981] shadow-inner">
                    {answer}
                </div>
            )}
        </div>
...
export default function GroutEstimatorApp() {
            ...
            {MATERIALS.map(mat => (
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all 
                  ${isQuantitySelected
                    ? (currentMat === mat.id 
                      ? 'bg-[#10B981] text-white shadow-md active:scale-[0.98] border-2 border-[#047857]' 
  ...
  const renderAreaList = (areas) => (
    <div className="space-y-4">
        {areas.map((area) => {
            ...
            const extraEntranceInfo = isEntranceAutoSelected ? <span className="block text-green-600 font-bold text-xs mt-0.5">바닥 2곳 선택 시 현관 무료 서비스!</span> : null;

            return (
                <div key={area.id} className={`flex flex-col p-4 rounded-xl border-2 transition duration-200 ${isSelected ? 'selection-selected shadow-md border-[#10B981]' : 'bg-white border-gray-200 hover:border-gray-300 shadow-sm'}`}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className={`p-3 rounded-lg shadow-sm ${isSelected ? 'bg-[#10B981] text-white' : 'bg-gray-100 text-gray-600'}`}><Icon size={20} /></div> 
                            <div>
  ...
  return (
    <div className={`min-h-screen bg-gray-50 font-sans pb-40`}>
      ...
      {/* ⭐️ [헤더] 화이트 바탕, 다크 폰트, 민트 로고 ⭐️ */}
      <header className="bg-white text-gray-800 sticky top-0 z-20 shadow-lg border-b border-gray-200">
        <div className="p-4 flex items-center justify-between max-w-md mx-auto">
          <div className="flex items-center"> 
            <h1 className="text-2xl font-extrabold text-[#10B981] tracking-wider">줄눈의미학</h1>
          </div>
          <div className='flex gap-2'> 
            <button 
              onClick={() => window.location.href = `tel:${PHONE_NUMBER}`} 
              className="text-xs bg-[#10B981] text-white px-3 py-1.5 rounded-full font-extrabold hover:bg-emerald-600 transition active:scale-95 shadow-md flex items-center"
            >
              <Phone size={14} className="inline mr-1" /> 상담
            </button>
            <button 
              onClick={() => window.location.reload()} 
              className="text-xs bg-gray-200 text-gray-700 px-3 py-1.5 rounded-full hover:bg-gray-300 transition active:scale-95 shadow-md flex items-center border border-gray-300"
            >
      ...
      <main className="max-w-md mx-auto p-4 space-y-8">
        ...
        {/* ⭐️ [동영상 섹션] ⭐️ */}
        <section className="bg-white rounded-xl shadow-md border border-gray-200 animate-fade-in overflow-hidden">
          <h2 className="text-xl font-extrabold flex items-center gap-2 p-4 text-gray-700 border-b border-gray-200 bg-gray-50">
                    ...
                    <button
                        ...
                        className={`flex-1 py-2 text-sm font-extrabold rounded-lg transition-all duration-300 shadow-sm active:scale-[0.99] ${
                            activeVideoId === video.id 
                                ? 'bg-[#10B981] text-white border-2 border-emerald-700' 
        ...
        {/* --- 1. 현장 유형 섹션 --- */}
        <section className="bg-white p-6 rounded-xl shadow-md border border-gray-200 animate-fade-in">
          <h2 className="text-xl font-extrabold flex items-center gap-2 mb-4 text-gray-700 border-b border-gray-200 pb-3">
            <Home className="h-6 w-6 text-[#10B981]" /> 1. 현장 유형을 선택하세요
          </h2 >
        ...
        <section className="bg-white p-6 rounded-xl shadow-md border border-gray-200 animate-fade-in">
          <h2 className="text-xl font-extrabold flex items-center gap-2 mb-4 text-gray-700 border-b border-gray-200 pb-3">
            <Hammer className="h-6 w-6 text-[#10B981]" /> 2. 줄눈소재 안내
          </h2 >
          <div className="space-y-4">
            {MATERIALS.map((item) => (
              <div key={item.id} className="animate-fade-in">
                <div onClick={() => setMaterial(item.id)} className={`flex flex-col p-4 rounded-xl border-2 transition-all duration-200 cursor-pointer selection-box active:scale-[0.99] ${item.id === material ? 'selection-selected shadow-md border-[#10B981]' : 'border-gray-300 bg-white hover:border-gray-400'}`}>
                  <div className="flex items-center justify-between">
                    <div className='flex items-center gap-3'>
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-2 transition ${item.id === material ? 'border-[#10B981]' : 'border-gray-400'}`}>
                        {item.id === material && <CheckCircle2 size={14} className="text-[#10B981]" />}
                      </div>
                ...
                {item.id === 'poly' && item.id === material && (
                  <div className="mt-3 ml-4 pl-4 border-l-4 border-gray-300 space-y-2 animate-slide-down bg-gray-50/70 p-4 rounded-xl shadow-inner">
                    <div className="text-sm font-bold text-gray-700 flex items-center gap-1"><Palette size={14} className='text-gray-500'/> 옵션 선택 (펄 유무)</div>
                    <div className="flex gap-3">
                      <button onClick={() => setPolyOption('pearl')} className={`flex-1 py-2 text-sm rounded-lg border transition-all shadow-sm ${polyOption === 'pearl' ? 'bg-[#10B981] text-white border-[#047857] font-extrabold' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-100'}`}>펄</button>
                      <button onClick={() => setPolyOption('no_pearl')} className={`flex-1 py-2 text-sm rounded-lg border transition-all shadow-sm ${polyOption === 'no_pearl' ? 'bg-[#10B981] text-white border-[#047857] font-extrabold' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-100'}`}>무펄</button>
                    </div>
                  </div>
                )}
                {item.id === 'kerapoxy' && item.id === material && (
                  <div className="mt-3 ml-4 pl-4 border-l-4 border-[#10B981] space-y-2 animate-slide-down bg-gray-50/70 p-4 rounded-xl shadow-inner"> 
                    <div className="text-sm font-bold text-gray-700 flex items-center gap-1"><Crown size={14} className='text-[#10B981]'/> 옵션 선택 (브랜드)</div> 
                    <div className="flex gap-3">
                      <button onClick={() => setEpoxyOption('kerapoxy')} className={`flex-1 py-2 text-sm rounded-lg border transition-all shadow-sm ${epoxyOption === 'kerapoxy' ? 'bg-[#10B981] text-white border-[#047857] font-extrabold' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-100'}`}>케라폭시</button> 
                      <button onClick={() => setEpoxyOption('starlike')} className={`flex-1 py-2 text-sm rounded-lg border transition-all shadow-sm ${epoxyOption === 'starlike' ? 'bg-[#10B981] text-white border-[#047857] font-extrabold' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-100'}`}>스타라이크</button> 
                    </div>
                  </div>
                )}
              </div>
        ...
        <section className="bg-white p-6 rounded-xl shadow-md border border-gray-200 animate-fade-in">
          <h2 className="text-xl font-extrabold flex items-center gap-2 mb-4 text-gray-700 border-b border-gray-200 pb-3">
            <Calculator className="h-6 w-6 text-[#10B981]" /> 3. 시공범위 선택
          </h2 >
          
          {/* A. 욕실 범위 */}
          <h3 className="text-lg font-extrabold flex items-center gap-2 mb-3 mt-4 text-gray-700">
            <Bath size={20} className="text-[#10B981]" /> A. 욕실 범위
          </h3>
          ...
          {/* B. 기타 범위 (현관/주방/베란다) */}
          <h3 className="text-lg font-extrabold flex items-center gap-2 mb-3 mt-4 text-gray-700">
            <LayoutGrid size={20} className="text-[#10B981]" /> B. 기타 범위
          </h3>
        ...
        <section className="bg-white p-6 rounded-xl shadow-md border border-gray-200 animate-fade-in">
          <h2 className="text-xl font-extrabold flex items-center gap-2 mb-4 text-gray-700 border-b border-gray-200 pb-3">
            <Eraser className="h-6 w-6 text-[#10B981]" /> 4. 실리콘 시공 (리폼)
          </h2 >
          <p className='text-sm text-gray-500 mb-4 bg-gray-50 p-3 rounded-lg border border-gray-200 font-bold'>줄눈 시공과 함께 진행 시 **할인 가격**이 적용됩니다. (3곳 이상 선택 시)</p>
          <div className="space-y-4">
        ...
        {/* --- 자주 묻는 질문 (FAQ) --- */}
        <section className="bg-white p-6 rounded-xl shadow-md border border-gray-200 animate-fade-in">
            <h2 className="text-xl font-extrabold text-gray-700 mb-2 flex items-center gap-2 border-b border-gray-200 pb-3">
                <HelpCircle className="h-6 w-6 text-[#10B981]"/> 자주 묻는 질문 (FAQ)
            </h2 >
        ...
        <div className="mt-6 pt-4 border-t border-gray-200">
          <button 
            ...
            className="w-full py-4 rounded-lg bg-[#10B981] text-white font-extrabold text-lg hover:bg-emerald-600 transition shadow-lg flex items-center justify-center gap-2 active:scale-95 border-2 border-[#047857]"
          >
      ...
      <>
        ...
        {/* ⭐️ [하단 견적 바] Dark 폰트, 민트/골드 강조 ⭐️ */}
        {hasSelections && (
            <div className="fixed bottom-0 left-0 right-0 bg-white shadow-2xl safe-area-bottom z-20 animate-slide-down border-t-2 border-[#10B981]">
                <div className="max-w-md mx-auto p-4 flex flex-col gap-2"> 
                    ...
                    <div className='flex items-center justify-between w-full text-gray-800'> 
                        ...
                        <div className='flex flex-col items-start gap-1'> 
                            <span className='text-sm font-semibold text-gray-600'>총 예상 견적</span>
                            <div className="flex items-end gap-1">
                                {/* 2. 최종 적용 가격 */}
                                <span className="text-4xl font-extrabold text-[#10B981] drop-shadow-sm">{calculation.price.toLocaleString()}</span>
                                <span className="text-xl font-extrabold text-gray-700">원</span>
                            </div>
                        </div>
                        ...
                        <div className='flex flex-col items-end justify-end h-full pt-1'> 
                            ...
                            {/* C. 패키지 적용 라벨 */}
                            {calculation.label && (
                                <div className="text-sm font-bold text-green-500 whitespace-nowrap">
                                    <Crown size={14} className='inline mr-1 text-green-500'/> {calculation.label}
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className='grid grid-cols-2 gap-3 mt-3'>
                        ...
                        <button 
                            ...
                            className={`w-full py-3.5 rounded-lg font-extrabold text-base transition-all 
                                bg-gray-800 text-white hover:bg-gray-700 active:bg-gray-900 shadow-lg 
                            `}
                        >
                        ...
                        <a 
                            ...
                            className={`w-full py-3.5 rounded-lg font-extrabold text-base transition-all 
                                bg-yellow-400 text-gray-800 hover:bg-yellow-500 active:bg-yellow-600 shadow-lg flex items-center justify-center
                            `}
                        >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white text-[#374151] w-full max-w-sm rounded-xl shadow-2xl overflow-hidden animate-slide-down border border-gray-200">
            <div className="bg-[#10B981] p-4 text-white flex justify-between items-center border-b border-emerald-600">
              <h3 className="font-extrabold text-xl flex items-center gap-2"><CheckCircle2 className="h-6 w-6 text-white" /> 줄눈의미학</h3> 
            ...
            <div className="p-5 text-gray-800 bg-white overflow-y-auto max-h-[70vh] custom-scrollbar"> 
              <div ref={quoteRef} id="quote-content" className="rounded-lg p-5 space-y-4 mx-auto border border-gray-200 bg-white" style={{ width: '320px' }}>
                
                {/* 헤더 및 로고 영역 */}
                <div className="flex flex-col items-center border-b border-gray-300 pb-3 mb-3">
                    <h1 className='text-2xl font-extrabold text-[#10B981] text-center'>ESTIMATE</h1>
                ...
                <div className="space-y-3 text-sm border-b border-gray-200 pb-3">
                    ...
                    {/* 패키지 포함 서비스 내역 */}
                    {calculation.isPackageActive && (
                        <div className="bg-green-50/70 p-2.5 rounded-lg border-l-4 border-[#10B981] text-xs font-semibold text-gray-700">
                            <p className='flex items-center gap-1 text-[#10B981] font-extrabold mb-1'>
                                <Crown size={14} className='text-[#047857]'/> {calculation.label} 
                            </p>
                        ...
                        {calculation.itemizedPrices
                            ...
                            return (
                                <div key={item.id} className="grid grid-cols-10 items-center text-gray-800 py-2 border-b border-gray-100 last:border-b-0">
                                    ...
                                    {/* 2. 소재 */}
                                    <span className="col-span-3 text-center font-extrabold text-xs text-green-600">
                                        {item.materialLabel}
                                    </span>
                ...
                <div className="pt-4 text-center border-t border-gray-300"> 
                    ...
                    <div className="flex justify-between items-end"> 
                        ...
                        <div className="text-right">
                            <span className="text-4xl font-extrabold text-[#10B981]">{calculation.price.toLocaleString()}원</span>
                        </div>
                    </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 space-y-2">
                    <div className='w-full py-2 px-2 text-center bg-gray-100 text-gray-700 rounded-lg font-bold text-xs shadow-sm flex items-center justify-center border border-gray-200'>
                        <TrendingUp size={14} className='inline mr-1 text-[#10B981]'/> 바닥 30x30cm, 벽면 30x60cm 크기 기준
                    </div>
                    <div className='w-full py-2 px-2 text-center bg-gray-100 text-gray-700 rounded-lg font-bold text-xs shadow-sm flex items-center justify-center border border-gray-200'>
                        <TrendingUp size={14} className='inline mr-1 text-[#10B981]'/> 재시공(셀프포함)은 별도문의
                    </div>
                    <div className='w-full py-2 px-2 text-center bg-gray-100 text-gray-700 rounded-lg font-bold text-xs shadow-sm flex items-center justify-center border border-gray-200'>
                        <TrendingUp size={14} className='inline mr-1 text-[#10B981]'/> 조각타일 및 대리석은 시공불가
                    </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-200">
                ...
                {soomgoReviewEvent && (
                    <div className='mb-3'>
                        {(() => {
                            ...
                            const activeClasses = "bg-[#10B981] text-white border-[#047857] hover:bg-emerald-600";
                            const inactiveClasses = "bg-yellow-400 text-gray-800 border-[#10B981] hover:bg-yellow-500";

                            const finalClasses = isApplied
                                ? activeClasses
                                : `${inactiveClasses} shine-effect`; 

                            const iconColorClass = isApplied ? 'text-white' : 'text-gray-800'; 
                ...
                <div className='grid grid-cols-3 gap-3'> 
                    ...
                    <button onClick={handleImageSave} className="flex items-center justify-center gap-1 bg-gray-700 text-white py-3 rounded-lg font-bold hover:bg-gray-800 transition text-sm active:scale-95 shadow-md"> 
                    ...
                    <button onClick={() => window.location.href = `tel:${PHONE_NUMBER}`} className="flex items-center justify-center gap-1 bg-[#10B981] text-white py-3 rounded-lg font-bold hover:bg-emerald-600 transition text-sm active:scale-95 shadow-md"> 
