...
    const totalAreaCount = Object.values(quantities).reduce((sum, count) => sum + count, 0); // 모든 항목의 총 수량 (할인 여부와 관계없음)
...
    if (totalAreaCount > 0) {
        estimatedHours = 4; // 최소 기본 시간 4시간

        // 욕실 바닥 당 1시간 추가 (총 2시간 한도)
        estimatedHours += Math.min(2, quantities['bathroom_floor'] * 1);

        // 벽 전체 시공 당 1.5시간 추가 (벽 전체 1개소당 1.5시간)
        estimatedHours += (quantities['master_bath_wall'] + quantities['common_bath_wall']) * 1.5;

        // 기타 공간 (베란다/세탁실/샤워부스/욕조벽) 2개 당 1시간 추가
        const otherAreas = (quantities['entrance'] || 0) + (quantities['balcony_laundry'] || 0) + (quantities['kitchen_wall'] || 0);
        estimatedHours += Math.floor(otherAreas / 2) * 0.5; // 2개소당 0.5시간

        // 거실 바닥은 시간이 많이 소요되므로 2시간 추가
        estimatedHours += (quantities['living_room'] > 0 ? 2 : 0);
        
        // 실리콘 시공 추가 (개소 당 0.3시간)
        estimatedHours += (quantities['silicon_bathtub'] + quantities['silicon_sink']) * 0.3;
        estimatedHours += (quantities['silicon_living_baseboard'] > 0 ? 0.5 : 0);

        // 시간을 0.5시간 단위로 올림 (작업 시간은 보통 넉넉하게 잡음)
        estimatedHours = Math.ceil(estimatedHours * 2) / 2;

        // 최대 8시간으로 제한
        estimatedHours = Math.min(estimatedHours, 8);
        estimatedHours = Math.max(4, estimatedHours); // 최소 4시간 보장
    }
    // ★★★ [수정된] 예상 시공 시간 계산 로직 끝 ★★★
...
        {/* ★★★ 최종 견적 하단 바 (수정된 estimatedHours 반영) ★★★ */}
...
                                    
                                    {/* 원가 또는 최종 적용 가격 */}
                                    <span className={`text-right w-2/5 font-bold text-sm ${isDiscounted ? 'text-gray-400 line-through' : 'text-blue-600'}`}>
...
                            
                            // 네이비 계열 색상 테마 적용 (디자인 통일)
                            const buttonClasses = isApplied
                                ? "bg-gray-700 shadow-gray-400/50 hover:bg-gray-800" // 적용된 상태
                                : "bg-indigo-600 shadow-indigo-400/50 hover:bg-indigo-700"; // 미적용 상태 (기본값 디자인)
                            
